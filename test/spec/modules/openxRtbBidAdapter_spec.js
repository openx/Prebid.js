...
import {spec, USER_ID_CODE_TO_QUERY_ARG} from 'modules/openxRtbBidAdapter.js';
...
const OPEN_RTB_ENDPOINT = 'rtb.openx.net/openrtbb/prebidjs';

describe.only('OpenxRtbAdapter', function () {
  ...
  describe('buildRequests for banner ads', function () {
    ...
    it('should send bid request to openx url via POST', function () {
      mockBidderRequest.bids = bidRequestsWithMediaTypes;
      const request = spec.buildRequests(bidRequestsWithMediaTypes, mockBidderRequest);
      expect(request[0].url).to.equal('https://' + OPEN_RTB_ENDPOINT);
    ...
    it('should send bid request with a mediaTypes specified with banner type', function () {
      ...
      expect(request[0].data.imp[1]).to.have.any.keys('banner');
    });
    ...
    it('should send platform id, if available', function () {
      mockBidderRequest.bids = bidRequestsWithPlatform;
      const request = spec.buildRequests(bidRequestsWithPlatform, mockBidderRequest);
      expect(request[0].data.ext.platformId).to.equal(bidRequestsWithPlatform[0].params.platform);
      expect(request[0].data.ext.delDomain).to.be.undefined;
    });
    ...
    it('should send out custom bc parameter, if override is present', function () {
      ...
      mockBidderRequest.bids = [bidRequest];
      const request = spec.buildRequests([bidRequest], mockBidderRequest);
      expect(request[0].data.ext.bc).to.equal(bidRequest.params.bc);
    });
    ...
    describe('when there is a consent management framework', function () {
      ...
      describe('when us_privacy applies', function () {
        ...
        it.only('should send a signal to specify that GDPR applies to this request', function () {
          bidderRequest.bids = bidRequests;
          const request = spec.buildRequests(bidRequests, bidderRequest);
          expect(request[0].data.imp[0].regs.ext.us_privacy).to.equal('1YYN');
          expect(request[0].data.imp[1].regs.ext.us_privacy).to.equal('1YYN');
        });
      });
